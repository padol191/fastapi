...
async def read_users(
    offset: int = 0, limit: int = 100, session: AsyncSession = Depends(get_db)
):
    """
    Retrieve users.
    """
...
async def create_user(
    ...
    Create new user.
    """
    user = await crud_user.get(session, login=user_in.login)
    if user is not None:
        raise HTTPException(
            status_code=409,
...
async def read_user(
    ...
    if not current_user: #TODO After role implementation
        raise HTTPException(
...
async def update_user(
    user_id: int, user_in: UserUpdate, session: AsyncSession = Depends(get_db)
):
    user = await crud_user.get(session, id=user_id)
    if user is None:
        raise HTTPException(
